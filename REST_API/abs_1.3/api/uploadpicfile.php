    ...
    if($clientfilename != '') {
        ...
        if(($access_token != null) || ($access_token != '')) {
           ...
           if(!$tokenvalid) {
               ...
               $em = new exceptionMgr(" ");
               $em->logInfo("uploadpicfile: Error: Invalid access token: ".$access_token);
           }
           else {
               ...
               if(!$childid) {
                   $responsedata = array(
                       'status' => "failed",
                       'description' => "Failed to retrieve the Child details for the given access token ($access_token)."
                   );
                   $em = new exceptionMgr(" ");
                   $em->logInfo("uploadpicfile: Error: Failed to retrieve the Child details for the given access token ($access_token).");
               }
               else if(($server_temp_filename == null) || ($server_temp_filename == '')) {
            ...
            $responsedata = array(
                'status' => "failed",
                'description' => "No access_token received."
            );
            $em = new exceptionMgr(" ");
            $em->logInfo("uploadpicfile: Error: No access_token received.");
        }
    }
    else {
