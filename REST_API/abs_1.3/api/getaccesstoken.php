 ...
 */
    ...
    if($childname && $phone && $deviceid){
	       ...
	       $accesstoken = getAccessTokenForChildForDevice($childname,$phone,$deviceid);	
	       	    
           if(!$accesstoken) {
               $responsedata = array(
                   'status' => "failed",
                   'description' => "Failed to retrieve access token for the Child for the device ($childname,$phone,$deviceid)."
               );
               $em = new exceptionMgr(" ");
               $em->logInfo("getaccesstoken: Error: Failed to retrieve access token for the Child for the device ($childname,$phone,$deviceid)");
           }
           else {
               $responsedata = array(
                   'status' => "success",
        ...
        $responsedata = array(
            'status' => "failed",
            'description' => "Inputs missing. Child name, phone number or the deviceid is missing"
        );
        $em = new exceptionMgr(" ");
        $em->logInfo("getaccesstoken: Error: Inputs missing. Child name, phone number or the deviceid is missing");
    }
